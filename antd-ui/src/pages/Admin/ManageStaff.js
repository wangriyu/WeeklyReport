...
const UpdateForm = Form.create()(props => {
  const { modalVisible, evtType, record, form, handleUpdate, handleCreate, handleModalVisible } = props;
        ...
        <FormItem label={'权限'}>
          {
            form.getFieldDecorator('role', {
              ...
              <Select placeholder={formatMessage({ id: 'form.role.placeholder' })}>
                <Option value="staff" key="staff">员工</Option>
                <Option value="leader" key="leader">主管</Option>
                <Option value="boss" key="boss">总工</Option>
              </Select>
            )
          }
        </FormItem>
...
@connect(({ mStaff, loading }) => ({
  mStaff,
  loading: loading.models.mStaff
}))
@Form.create()
class ManageStaff extends PureComponent {
  ...
  handleCreate = (payload) => {
    this.setState({ modalVisible: !this.state.modalVisible, record: {} });
    this.props.dispatch({
      type: 'mStaff/createStaff',
      payload: payload
    })
  };

  render() {
    const { mStaff: { list }, loading } = this.props;
    ...
    return (
      <Card bordered={false}>
        <div style={{marginBottom: '10px', display: 'flex', justifyContent: 'space-between'}}>
          ...
          <Button type='primary' onClick={() => this.setState({ modalVisible: true, evtType: 'create' })}>新建</Button>
        </div>
        <Table
