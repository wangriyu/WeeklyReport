...
import {
  Input, Modal, Form, InputNumber, Select, DatePicker,
} from 'antd';
...
const AnnuallyModal = (props) => {
  const { modalVisible, evtType, record, form, handleUpdateItem, handleCreateItem, handleCreatePlan, handleModalVisible } = props;
  ...
  const handleOk = () => {
    form.validateFields((err, fieldsValue) => {
      ...
      switch (evtType) {
        case 'createPlan':
          fieldsValue['title'] = fieldsValue.title + '';
          handleCreatePlan(fieldsValue);
          break;
        case 'createItem':
          handleCreateItem(fieldsValue);
          break;
        case 'updateItem':
          fieldsValue['id'] = record.id;
          handleUpdateItem(record.pid, fieldsValue);
          break;
      }
    });
  };
            ...
            <FormItem label={'紧急程度'}>
              {
                form.getFieldDecorator('priority', {
                  ...
                  <Select placeholder='选择优先级'>
                    ...
                    <Option value={2} key={2}>低</Option>
                    <Option value={3} key={3}>中</Option>
                    <Option value={4} key={4}>高</Option>
                  </Select>
                )
              }
            </FormItem>
            <FormItem label={'计划完成时间'}>
              {
                form.getFieldDecorator('deadline', {
                  initialValue: !record.deadline ? null : moment(record.deadline, 'YYYY-MM-DD HH:mm:ss'),
                })(
